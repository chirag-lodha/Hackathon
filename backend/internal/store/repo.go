...
func (r *Repo) GetSession(id uint) (*Session, error) {
	var sess Session
	if err := r.db.First(&sess, id).Error; err != nil {
		return nil, err
	}
	return &sess, nil
}
...
func (r *Repo) ImageDone(id, path, eenTs string) error {
	return r.db.Model(&Image{}).Where("id = ?", id).
		Updates(map[string]any{"state": StateSuccess, "path": path, "een_ts": eenTs}).Error
}

func (r *Repo) ImageFailed(id, msg string) error {
	return r.db.Model(&Image{}).Where("id = ?", id).
		Updates(map[string]any{"state": StateFailure, "error": msg}).Error
}
